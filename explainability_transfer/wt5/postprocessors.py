# coding=utf-8
#
# Extended and adopted from https://github.com/google-research/google-research/blob/master/wt5/wt5/postprocessors.py
# 
# Original copyright:
...
def abstractive_explanations(output,
  ...
  """
  output = (output.replace("<s>", "").replace("</s>", "").replace("<unk>", "")
            .replace("<pad>", "").replace("<mask>", ""))
...
def extractive_explanations(output,
  ...
  for e in explanations:
    ...
    if overlap_tuple:
      ...
      if is_target:
        logging.warning("Rationale: %s from ground truth not found in "
                        "input example." % e)
  ...
  return {
      ...
      "explanations_all": raw_explanations,
  }
